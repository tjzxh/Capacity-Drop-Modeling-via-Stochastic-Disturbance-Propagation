...
# filter out the density that fall into the critical range (23~33)
# density_list = [i for i in density_list if i <= 30 or i >= 40]
real_density_list = []
...
mean_model.set_param_hint('vm', value=60, min=50, max=100)
mean_model.set_param_hint('a', value=0.1, min=0.01, max=0.15)
mean_model.set_param_hint('b', value=-0.5, min=-1, max=-0.01)
mean_result = mean_model.fit(mean, x=real_density_list, weights=sample_weights)  #
# l2 result km/h
# ==============================================================================
#                  coef    std err          t      P>|t|      [0.025      0.975]
# ------------------------------------------------------------------------------
# const         -0.9602      0.079    -12.125      0.000      -1.118      -0.802
# x1            -0.1920      0.020     -9.589      0.000      -0.232      -0.152
# ==============================================================================
...
print(f'mean MAPE: {mean_mape:.1%} and PICP: {avg_picp:.2%}')
# mean MAPE: 9.5% and PICP: 100.00%
...
# mean R-squared:  0.7726259278241852
...
# get the critical density within the range of [30, 40] points alpha=0.5
kc_range = np.where((density >= 26) & (density <= 42))
rest_range = np.where((density < 26) | (density > 42))
ax0.scatter(density[kc_range], flow[kc_range], c='b', alpha=0.1, s=30)
ax0.scatter(density[rest_range], flow[rest_range], c='b', label='Zen data', s=30)
ax0.set_xlabel("$k$ (veh/km/lane)")
ax0.set_ylabel("$Q$ (veh/h/lane)")
# ax0.set_xlim([0, 50])
ax0.set_xticks(np.arange(0, 85, 10))
...
print(f"the error percentage: {100 * abs(np.max((y1 + y2) / 2) - 1920.0) / 1920.0}%")
# mean R-squared:  0.7903279139643608
# a, b, Capacity = 0.06284705525216983, 0.2056057454419577, 1900.506826249206
# the error percentage: 1.015269466187192%
